...
import org.rmj.cas.parameter.agent.XMCard;
...
public class CreditcardController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        if (poData == null){
            ShowMessageFX.Warning("Credit card data is not set.", pxeModuleName, "Please inform MIS Department.");
            System.exit(0);
        }
        ...
        txtCardType.focusedProperty().addListener(txtField_Focus);
        txtCardNoxx.focusedProperty().addListener(txtField_Focus);
        txtApprovNo.focusedProperty().addListener(txtField_Focus);
        txtBatchNox.focusedProperty().addListener(txtField_Focus);
        ...
        txtCardType.setOnKeyPressed(this::txtField_KeyPressed);
        txtCardNoxx.setOnKeyPressed(this::txtField_KeyPressed);
        txtApprovNo.setOnKeyPressed(this::txtField_KeyPressed);
        txtBatchNox.setOnKeyPressed(this::txtField_KeyPressed);
    ...
    private boolean isEntryOK(){
        ...
        if (poData.getCardIDxx()== null || poData.getCardIDxx().equals("")){
            ShowMessageFX.Warning(getStage(), "Invalid card detected.", pxeModuleName, "Please verify your entry.");
        ...
        if (poData.getBatchNox()== null || poData.getBatchNox().equals("")){
            ShowMessageFX.Warning(getStage(), "Invalid batch number detected.", pxeModuleName, "Please verify your entry.");
            txtBatchNox.requestFocus();
            return false;
        }
    ...
    private void txtField_KeyPressed(KeyEvent event) {
        ...
        switch(txtField.getId().toLowerCase()){
            ...
            case "txtcardtype":
                switch (event.getCode()){
                case F3:
                    XMCard loCard = new XMCard(poGRider, poGRider.getBranchCode(), true);
                    JSONObject loJSON = loCard.searchCard(lsValue, false);
                    
                    poData.setCardIDxx("");
                    if (loJSON != null){
                        poData.setCardIDxx((String) loJSON.get("sCardIDxx"));
                        txtField.setText((String) loJSON.get("sCardName"));
                        CommonUtils.SetNextFocus(txtField);
                    }
                }
                break;
    ...
    private void loadData(){
        ...
        txtBatchNox.setText(poData.getBatchNox());
    ...
    private void loadCard(){        
        if (!poData.getCardIDxx().equals("")){
            XMCard loCard = new XMCard(poGRider, poGRider.getBranchCode(), true);
            JSONObject loJSON = loCard.searchCard(poData.getCardIDxx(), true);
            
            if (loJSON != null)
                txtCardType.setText((String) loJSON.get("sCardName"));
            else
                txtCardType.setText("");
        } else txtCardType.setText("");
    }
    
    private void loadTerm(){
        if (!poData.getTermCode().equals("")){
            XMTerm loTerm = new XMTerm(poGRider, poGRider.getBranchCode(), true);
            JSONObject loJSON = loTerm.searchTerm(poData.getTermCode(), true);
